<script lang="ts">
  ...
  $: outline_property_array = Array.from($outline_property.values());
...
>
  {#if show_code}
    <div
      id="first"
      class={`my-auto w-[400px] h-[400px]  ${$padding} ${bg_color} ${$border_width} ${border_color} ${$border_radius} ${$ring_width} ${ring_color}  ${outline_property_array.join(
        " "
      )} ${outline_color}`}
    />
  {:else}
    <div class="mockup-code">
      ...
      {`my-auto w-[400px] h-[400px] ${$padding} ${bg_color} ${$border_width} ${border_color} ${$border_radius} ${$ring_width} ${ring_color} ${$outline_property}`}
        </code>
      </pre>
    </div>
  {/if}
